...
Plant pathogens and pests remain a major constraint on agricultural production, reducing both yield and quality and creating strong demand for timely, site-specific crop-health management \citep{Savary2019GlobalBurden}. Grapevine production is particularly exposed to disease risks because major foliar and fruit diseases, including downy mildew, powdery mildew, gray mold, and black rot, respond dynamically to local weather, host growth stage, cultivar susceptibility, canopy condition, and recent plant-protection operations \citep{Peng2024DownyMildew,Kunova2021PowderyMildew,Rahman2024GrayMold}. Gray mold and other bunch rots affect grape and wine quality as well as yield, while powdery mildew and downy mildew management remains closely linked to fungicide timing and resistance-aware disease control \citep{Steel2013BunchRots,Kunova2021PowderyMildew}. In practice, disease control is also shaped by environmental and regulatory pressure to reduce unnecessary pesticide inputs while maintaining fruit quality and yield stability \citep{Mian2021DownyMildewDSS}. Disease management in vineyards is therefore a continuous decision process, not only a diagnostic problem, because growers must interpret infection-conducive weather, phenological susceptibility, visible symptoms, treatment windows, and fungicide protection history within a short operational period \citep{Orlandini1993PLASMO}.

Several technical approaches have been developed to support this process, especially weather-driven disease models, phenological models, decision-support systems, and image-based diagnosis methods \citep{OrtegaFarias2019Phenology,Xie2020GrapeDetector}. Weather-driven disease models have long been used to simulate pathogen development and guide fungicide timing in vineyards \citep{Orlandini1993PLASMO,Orlandini1995DownyMildewProgram}. For example, PLASMO simulated grapevine downy mildew development from temperature, leaf wetness, relative humidity, and treatment information, and was designed for direct use in vineyard management \citep{Orlandini1993PLASMO}. Later model-based systems and evaluations showed that downy mildew prediction remains sensitive to regional climatic conditions and therefore requires field-level calibration, validation, and operational interpretation rather than simple transfer of generic infection rules \citep{Puelles2024DownyMildewModels}. Vineyard decision-support systems such as vite.net combined real-time monitoring, data storage, mechanistic models, alerts, and web-based decision support, explicitly addressing the implementation problem that has limited the sustained use of many agricultural DSSs \citep{Rossi2014ViteNet,McCown2002DSSComplexity}. In parallel, grapevine growth-stage scales and phenology models provide a formal basis for linking crop development to management timing, including BBCH or modified Eichhorn-Lorenz stage descriptions and cumulative-degree-day model formulations \citep{Lorenz1995BBCH,Coombe1995ModifiedEL,Molitor2014HighResolutionPhenology}. Recent cultivar-scale and multi-stage phenology models further indicate that temperature-driven development can be represented quantitatively across cultivars, although calibration data and regional context affect model behavior \citep{Caffarra2014PhenologyComparison,Molitor2020UniPhen}.

Recent advances in computer vision have added another important source of field evidence for crop-health assessment \citep{Xie2020GrapeDetector,Liu2020GrapeLeafIdentification,Shi2023SeverityAssessment}. Deep convolutional networks, vision transformers, and object-detection models have been applied to grape leaf disease classification, downy mildew detection, and lesion localization under natural or semi-natural imaging conditions \citep{Zhang2022DownyMildewDetection,Kunduracioglu2024GrapeDeepLearning,Liu2024FusionTransformerYOLO}. Disease-severity studies further emphasize the value of pixel-level lesion and leaf segmentation for quantifying symptom intensity instead of reporting only a class label \citep{Shi2023SeverityAssessment}. These methods can reduce dependence on manual visual inspection and support fast symptom-level assessment in the field \citep{Xie2020GrapeDetector,Kunduracioglu2024GrapeDeepLearning}. However, image diagnosis alone does not explain whether weather and phenology are favorable for infection, whether a treatment window is open, or whether previous fungicide applications still provide protection, because those decisions depend on weather-driven epidemiology, growth stage, and treatment history \citep{Rossi2014ViteNet}.

The principal gap is therefore one of system integration, because prior work often advances disease forecasting, phenology simulation, image diagnosis, or farm recording as separate technical functions \citep{Fountas2015FMIS,Tummers2019FMISReview}. Farm management information systems have evolved from record-keeping tools toward complex decision-support environments, yet reviews continue to report persistent obstacles such as data fragmentation, limited interoperability, usability constraints, and weak coupling between analytical functions and farm operations \citep{Fountas2015FMIS,Tummers2019FMISReview}. Agricultural DSS research has also emphasized that model value depends on delivery, usability, trust, and fit with decision contexts rather than on predictive algorithms alone \citep{McCown2002DSSComplexity,Rose2016DecisionSupportTools}. In practice, a grower may receive a risk index in one tool, record spraying in another, and inspect disease symptoms through a separate image-recognition workflow, reproducing the fragmentation described for agricultural information systems \citep{Tummers2019FMISReview}. This separation limits the conversion of model outputs into field tasks and the reuse of completed operations in subsequent risk calculations, a challenge that vineyard DSS work has addressed through closer coupling between monitoring, models, warnings, and management advice \citep{Rossi2014ViteNet,Mian2021DownyMildewDSS}. End-to-end agricultural platforms have shown the value of integrating data acquisition, ingestion, storage, analytics, and user-facing decision support into a scalable workflow; a similar platform perspective is needed for vineyard disease warning and mobile field management \citep{Bakir2026PrecisionRanchingPlatform,Fountas2015FMIS}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/fig01_system_architecture.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/fig02_interaction_flow.png}
...
\caption{Disease-model simulation workflow in GrapeMaster. Hourly weather, crop growth stage, cultivar information, fungicide history, and target disease settings are converted into weather response, host correction, protection correction, and risk-timeline outputs, which support disease-risk display, field-risk interpretation, protection-time tracking, recommendations, and treatment-window generation.}
\label{fig:disease_model_workflow}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/fig04_input_optimization.png}
...
GrapeMaster has operated from 2024 to 2026 in multiple viticultural regions in China, including Yangling in Shaanxi Province, Nanning, Du'an of Hechi, and Xing'an of Guilin in Guangxi Province. These sites represent different climatic and management settings and provide an initial basis for examining whether the platform can support disease warning and spray-timing decisions beyond a single vineyard context. Figure~\ref{fig:trial_distribution} shows the geographic distribution of test-account fields used for platform-function testing and experimental data sites used for phenology and disease evidence. During deployment, the platform was used for spore monitoring, disease warning, and plant-protection scheduling. The weather-driven warning workflow mainly simulated grapevine downy mildew and powdery mildew risks, corresponding to \texttt{PLASVI} and \texttt{UNCINE} in the disease-model service. Field observations indicated that initial spore capture preceded the first visible disease symptoms by approximately one to two months, and that the first major spore peak broadly coincided with the period of rapid disease development. These observations suggest that the platform can provide a warning signal earlier than symptom-based scouting while retaining temporal consistency with subsequent epidemic acceleration.

Figure~\ref{fig:monitoring_instruments} shows representative field-monitoring instruments used during deployment, including spore traps and an automatic weather station. These instruments provided the field-level monitoring context for spore capture and local weather acquisition used by the platform workflow.
...
\caption{Geographic distribution of GrapeMaster test-account fields and experimental data sites in China. Test-account fields indicate fields associated with accounts used for platform-function testing, while experimental data sites indicate field locations used as sources of phenology and disease evidence.}
\label{fig:trial_distribution}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/fig06_monitoring_instruments.png}
\caption{Field-monitoring instruments used in GrapeMaster deployment. Panels show (a) a spore trap, (b) an automatic weather station, and (c) slide replacement for a vineyard-installed spore trap.}
\label{fig:monitoring_instruments}
\end{figure}

\section{Features and Functionalities}

GrapeMaster provides user-facing functions through mobile workspaces for vineyard field management, crop-season status review, task management, and image-based disease assistance. These workspaces share a crop-season context, allowing weather information, phenological status, disease warnings, image evidence, field observations, and completed operations to be displayed and updated for the same vineyard season. The following subsections focus on the functions most directly related to vineyard disease warning and precision management.

\subsection{Vineyard Field and Crop-Season Management}

The field-management function provides the spatial entry point of the platform. Users can create, inspect, edit, share, delete, recover, and select vineyard fields. During field creation, the mobile client supports map-assisted input and records the field name, boundary geometry, area, centroid longitude and latitude, administrative region, and a generated boundary preview. Each field is displayed as a selectable field card in the mobile workspace, from which users can access crop seasons, weather, warnings, notes, tasks, and field settings. Representative field-management interfaces are shown in Figure~\ref{fig:field_management_interfaces}.
...
Within the crop-season workspace, users can view the selected cultivar and cultivation method, inspect the current growth-stage context, open disease-warning information, review weather details, and enter task or note pages without manually reselecting the field. Ordinary crop notes, growth-stage notes, and disease-incidence notes can be stored with dates, text, images, and crop-season context, providing field evidence that helps interpret model warnings and later management outcomes. The workspace therefore makes the crop season the operational decision context and keeps weather, disease, notes, and tasks attached to the same field-season object.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/field_management_interfaces.png}
\caption{Representative field-management interfaces in GrapeMaster: (a) user login, (b) field list and weather overview, and (c) map-based field-boundary editing.}
\label{fig:field_management_interfaces}
\end{figure}
...
The spraying-suitability function converts short-term weather conditions into field-operation guidance. It considers temperature, precipitation, wind speed, relative humidity, near-future rainfall, cultivation method, sunrise, and sunset. The mobile client can therefore display both disease pressure and near-term spraying feasibility. This function is relevant when a treatment window is open but rainfall, wind, or unsuitable timing may reduce spray effectiveness or prevent safe execution. Example interfaces for weather, spraying suitability, disease-risk, and growth-stage display are shown in Figure~\ref{fig:weather_risk_interfaces}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/weather_phenology_interfaces.png}
\caption{Weather, operation-suitability, and crop-season status interfaces: (a) weather detail and multi-day forecast, (b) hourly weather and spraying-suitability display, and (c) field-level disease-risk and growth-stage forecasts.}
\label{fig:weather_risk_interfaces}
\end{figure}
...
Figure~\ref{fig:seasonal_disease_risk_timeline} shows representative daily disease-model outputs from test crop seasons. The three horizontal tracks in each panel separate downy mildew risk, powdery mildew risk, and field-level integrated risk. The color sequence shows how the model moves from non-seasonal periods to unfavorable, favorable, optimal, and, when fungicide records are present, protected states. This timeline format makes the temporal structure of warnings visible and shows how a fungicide application can modify subsequent model risk states.
...
The warning function is connected with notifications and field feedback. When phenology changes or disease-risk states trigger an alert, the backend generates user-specific notices. These notices can be reviewed, marked as read, or used as entry points for related operations such as task creation. User-entered disease notes and disease reports provide an observation channel that can be compared with simulated risk periods, supporting later model validation and expert review. In this way, disease-risk information is exposed through both the field workspace and the notification workflow, while field observations can be returned to the system for management review. Figure~\ref{fig:alert_notification_interfaces} illustrates the alert and notification workflow.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/disease_risk_timeline.png}
\caption{Seasonal disease-risk output timeline generated from disease-model test outputs. Panels show (a) the Mingyang 2024 crop season, (b) the GXG-233 crop season without fungicide history, and (c) the GXG-233 crop season with fungicide history. Each panel contains daily risk states for downy mildew, powdery mildew, and field-level risk. Colors indicate the model risk code: not seasonal, unfavorable, favorable, optimal, or protected. The dashed vertical line in panel (c) marks the fungicide application date used by the model to produce protected risk states.}
\label{fig:seasonal_disease_risk_timeline}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../fig/alert_notification_interfaces.png}
\caption{Warning and notification interfaces in GrapeMaster: (a) disease alert dialog, (b) irrigation notice with task-creation action, and (c) notification list.}
\label{fig:alert_notification_interfaces}
\end{figure}
...
\caption{Task-management and plant-protection input interfaces: (a) task list with pending and completed operations, (b) fungicide and pesticide product selection, and (c) sprayer-tank mixing and dosage calculation.}
\label{fig:task_management_interfaces}
\end{figure}

\subsection{Image-Based Disease Assistance and Severity Estimation}

The recognition module supports field-image-based assistance. Users can select an image from the camera or gallery, preview it in the mobile client, and upload it for recognition. Before upload, the image can be resized and compressed to reduce transmission load. The backend image-recognition service classifies grape images into disease or healthy categories and returns corresponding diagnostic or management text to the user. The implemented classifier currently returns grape leaf blight, grape black rot, grape Esca or black measles, healthy grape, and grapevine yellowing categories. This module is therefore presented as an auxiliary visual-diagnosis and field-evidence function, whereas the main weather-driven warning workflow focuses on downy mildew and powdery mildew risk.
